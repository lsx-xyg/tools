...
全站壳（导航 + 首页 + 工具页框架），Operate 模式为主，首页带轻量发现（Persuade）元素。首个工具页：文本/文件互传。
...
免登录、免费、内容 24 小时自动清除、文件最多下载 10 次。真实功能，不编造宣传。

## Constraints

菜单化架构，新增工具 = 新页面 + 新菜单项；运行在 Cloudflare Workers 免费额度（KV/R2）；现代、克制、操作舒适；界面以中文为主，品牌词中英文并存。
...
**FIRST VIEWPORT**：品牌行 + 主题切换；仪表阵列（工具卡片带状态灯与编号读数），互传工具作为第一台仪表置前；下方一行「24 小时自动清除 · 免登录 · 免费」。
