...
在线工具箱：一个可不断添加小工具的站点，当前已有 24 个工具（T-01 文本/文件互传 ～ T-24 请求头解析），见 `lib/tools.ts` 注册清单。核心工具「文本/文件互传」：发送端输入文本或上传文件后获得 6 位提取码，接收端输入提取码或扫码即可取回；支持 WebRTC 点对点在线直传（内容不落服务器）与离线暂存（保留时长 / 下载次数可自定义，到期自动销毁）。
